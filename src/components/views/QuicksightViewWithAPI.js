  ...
  useEffect(() => {
    const fetchUrls = async () => {
      try {
        ...
        const globalTBUrls = await Promise.all(
          globalTBParams.map(async (params) => {
        ...
        const countryUrls = await Promise.all(
          countryParams.map(async (params) => {
            const response = await axios.get('https://v83joq8cn3.execute-api.us-east-1.amazonaws.com/visual/api/getUrl', {
              params: params,
            });
            return response.data.embedUrl;
        ...
        const demographicsUrls = await Promise.all(
          demographicsParams.map(async (params) => {
            const response = await axios.get('https://v83joq8cn3.execute-api.us-east-1.amazonaws.com/visual/api/getUrl', {
              params: params,
            });
            return response.data.embedUrl;
        ...
        const budgetExpenditureUrls = await Promise.all(
          budgetExpenditureParams.map(async (params) => {
            const response = await axios.get('https://v83joq8cn3.execute-api.us-east-1.amazonaws.com/visual/api/getUrl', {
              params: params,
            });
            return response.data.embedUrl;
        ...
        urlsData.quickSightGlobalTB = globalTBUrls;
        urlsData.quickSightCountry =  countryUrls;
        urlsData.quickSightDemographics = demographicsUrls;
        urlsData.quickSightBudgetExpenditure = budgetExpenditureUrls;

        setViewUrls(urlsData);
      } catch (error) {
        setErorr(error);
      }
    };
